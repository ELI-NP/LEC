...
c
     ...
     &	   + 17.d0*rho**4.d0*C5/16.d0
     &         - 3.d0*rho**6.d0*C6/8.d0
     &         + rho**8.d0*C7/32.d0
      TermB1 = rho*rho*S2/2.d0 - rho**4.d0*S3/4.d0
      TermB2 = -S2/8.d0 + rho*rho*S3/4.d0
     &         + 5.d0*rho**4.d0*S4/16.d0
     ...
     &	   + rho**8.d0*C7/32.d0
c
      E1temp = xxi*(eps*C1 + eps**3.d0*(TermE4)
     &         + eps**5.d0*(TermE5))
      E2temp = S0 + eps**2.d0*(TermE1)
     &         + eps**4.d0*(TermE2)
      E3temp = xxi*nu*(eps**2.d0*S2 + eps**4.d0*(TermE3))
      B1temp = nu*(eps*C1 + eps**3.d0*(TermB3)
     &         + eps**5.d0*(TermB4))
      B2temp = 0.d0
